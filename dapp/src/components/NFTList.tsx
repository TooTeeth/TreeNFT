...
function NFTList({ signer, treeNFTContract }: NFTListProps) {
  ...
  const getMyNFTs = async () => {
    ...
    try {
      ...
      for (let i = 0; i < Number(balance); i++) {
        nftPromises.push(treeNFTContract.tokenOfOwnerByIndex(signer.address, i));
      }
  ...
  return (
    <>
      <Flex {...containerStyle} mt={12} flexDirection="column" gap={4} alignItems="center">
        <Button colorPalette="green" onClick={handleClaimRewards} loading={isLoading} loadingText="수확중">
          수확하기
        </Button>
        <Grid as="ul" gridTemplateColumns={["repeat(1, 1fr)", "repeat(2, 1fr)", "repeat(3, 1fr)", "repeat(4, 1fr)"]} justifyItems="center" gap={2}>
          {tokenURIs.map((v, i) => (
            <NFTCard key={i} tokenId={myNFTs[i]} tokenURI={v} treeNFTContract={treeNFTContract} isOpen={isOpen} />
          ))}
        </Grid>
      </Flex>
      <SuccessDialog isOpen={isOpen} setIsOpen={setIsOpen} title="Fruit 토큰 수확" message="수확이 완료되었습니다." />
    </>
  );
}
